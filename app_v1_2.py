...
with st.sidebar:
    ...
    if st.button("Çıkış yap", width="stretch"):
        for key in ["trendyol_orders", "profitgo_filename"]:
...
if page == "Genel Bakış":
    ...
    with c1:
        if summary:
            ...
            st.button("Rapor yüklemeye git →", type="primary", on_click=_go_to, args=("Rapor Yükle",))
    with c2:
    ...
    st.markdown('<div class="pg-card"><div class="pg-eyebrow">YENİ ANALİZ</div><h3>Trendyol raporunu içeri al</h3><p>Dosya bu canlı demo oturumunda analiz edilir. Kalıcı müşteri depolaması V1.3 demo sürümünde kullanılmaz.</p></div>', unsafe_allow_html=True)
    ...
    if not has_data:
        ...
        st.button("Rapor yüklemeye git →", type="primary", on_click=_go_to, args=("Rapor Yükle",), key="audit_to_upload")
    else:
    ...
    if summary:
        ...
        st.button("Rapor yüklemeye git →", type="primary", on_click=_go_to, args=("Rapor Yükle",), key="reports_to_upload")
    ...
    st.markdown('<div class="pg-card"><div class="pg-eyebrow">ABONELİK</div><h3>ProfitGO planın</h3><p>V1.3 demo plan ekranı hazır; gerçek abonelik ve ödeme altyapısı sonraki production aşamasında bağlanacak.</p></div>', unsafe_allow_html=True)

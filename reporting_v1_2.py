...
HISTORY_FILE = HISTORY_DIR / "report_history.json"
...
def build_summary(df: pd.DataFrame, filename: str = "Trendyol Raporu") -> dict:
    ...
    # ProfitGO V6 logic summarized for a product-level overview.
    if penalty_mask.any():
        p = df.loc[penalty_mask].copy()
        p_penalty = pd.to_numeric(p.get("Ceza Bedeli", 0), errors="coerce").fillna(0).abs()
        p_status = p.get("Sipariş Statüsü", pd.Series([""] * len(p), index=p.index)).astype(str).str.replace("İ", "i").str.replace("I", "ı").str.lower()
